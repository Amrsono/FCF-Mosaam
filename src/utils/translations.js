export const translations = {
  en: {
    // Sidebar
    inventory: "Orders Inventory",
    customers: "Customers",
    sla: "Delivery SLA",
    penalties: "Parked Penalties",
    basata: "Basata POS",
    jumia: "Jumia",
    bosta: "Bosta Orders",
    returned: "Returned to Jumia",
    analytics: "Analytics Insights",
    signOut: "Sign Out",
    jumiaStation: "Jumia Station V3",
    fcf: "FCF",
    mosaam: "Mosaam",
    admin: "Admin",
    staff: "Staff",
    callsLog: "Calls Log",
    rechargeApprovals: "Recharge Approvals",
    rechargeApprovalsDesc: "Manage and approve balance/limit recharge requests",
    logs: "System Logs",
    user: "User",
    action: "Action",
    details: "Details",
    allActions: "All Actions",
     allUsers: "All Users",
    manageUsers: "Manage Users",
    userBranchAssignment: "User Branch Assignment",
    assignBranch: "Assign Branch",
    assignedBranch: "Assigned Branch",
    selectBranch: "Select Branch",
    successUpdateUser: "User branch updated successfully",

    // Common
    confirm: "Confirm",
    cancel: "Cancel",
    search: "Search...",
    loading: "Loading FCF Mosaam...",
    successfullyLogged: "Successfully logged!",
    errorLogging: "Error logging transaction",
    noData: "No data found.",
    date: "Date",
    amount: "Amount",
    status: "Status",
    actions: "Actions",
    exportReport: "Export Report",
    importData: "Import Data",
    total: "Total",
    save: "Save",
    delete: "Delete",
    edit: "Edit",
    view: "View",
    all: "All",
    from: "From",
    to: "To",
    custom: "Custom Range",

    // Orders Tab
    receiveNewOrder: "Receive New Order",
    bulkImport: "Bulk Import (Excel/CSV)",
    orderId: "Order ID",
    customer: "Customer",
    phone: "Phone",
    description: "Description",
    value: "Value",
    category: "Category",
    receivedAt: "Received At",
    markPickedUp: "Mark Picked Up",
    markReturned: "Mark Returned",
    daysInInv: "Days in Inv.",
    penalty: "Penalty",
    inventorySummary: "Inventory Summary",
    parkedOrders: "Parked Orders",
    totalValue: "Total Value",
    totalPenalties: "Total Penalties",
    packageSize: "Package Size",
    small: "Small",
    medium: "Medium",
    big: "Big",

    // Customers Tab
    customerDirectory: "Customer Directory",
    addCustomer: "Add New Customer",
    name: "Name",
    email: "Email",
    address: "Address",
    tier: "Tier",
    deliveries: "Deliveries",
    bostaDeliveries: "Bosta Deliveries",
    newCustomer: "New Customer",
    loyaltyTier: "Loyalty Tier",
    customerInfo: "Customer Information",

    // Basata Tab
    logPosTransaction: "Log POS Transaction",
    popularQuickActions: "Popular Quick Actions (Mobile Top-ups)",
    allServiceCategories: "All Service Categories",
    transactionReport: "Transactions Report",
    serviceProvider: "Service Provider",
    transactionId: "Transaction ID",
    paymentMethod: "Payment Method",
    percentage: "Percentage %",
    totalVolume: "Total Volume",
    method: "Method",
    service: "Service",
    confirmLogging: "Confirm Logging",
    cash: "Cash",
    card: "Card",
    wallet: "Digital Wallet",
    other: "Other",
    bankingAndCash: "Banking & Cash",
    cashDeposit: "Cash Deposit",
    cashWithdrawal: "Cash Withdrawal",
    visa: "Visa",
    creditCard: "Credit Card",
    jumiaPay: "Jumia Pay",
    eltalg: "eltalg",
    tegara: "tegara",
    mostashfa: "mostashfa",

    // Bosta Tab
    bostaInventory: "Bosta Orders Inventory",
    receiveBostaOrder: "Receive Bosta Order",

    // SLA Tab
    slaMonitoring: "SLA Monitoring & Overdue Tracking",
    slaAlerts: "SLA Alerts",
    critical5Days: "Critical (5+ Days)",
    warning3Days: "Warning (3-4 Days)",
    onTrack: "On Track",

    // Parked Penalties Tab
    parkedPenalties: "Parked Orders & Daily Penalties",
    parkedDescription: "Overview of orders currently parked at the station and their accumulated late pickup penalties.",

    // Returned Tab
    returnedOrders: "Returned to Jumia",
    returnDate: "Return Date",

    // Customer Returns Section (in Orders Tab)
    customerReturns: "Customer Returns",
    customerReturnsDesc: "Packages returned by customers to the station, pending return to Jumia.",
    receiveCustomerReturn: "Receive Customer Return",
    returnReason: "Reason",
    atStation: "At Station",
    returnedToJumia: "Returned to Jumia",
    markReturnedToJumia: "Mark Returned to Jumia",
    pendingReturns: "Pending Returns",

    // Analytics Tab
    performance: "Performance",
    daily: "Daily",
    weekly: "Weekly",
    monthly: "Monthly",
    totalStationRevenue: "Total Station Revenue",
    revenueStreams: "Revenue Streams Breakdown",
    ordersComparison: "Jumia vs Bosta Orders Comparison",
    ordersStatusDist: "Full Orders Status Distribution",
    slaHealth: "Jumia SLA Health",
    basataRevenueCat: "Basata Revenue by Category",
    topBasataServices: "Top Basata Services by Volume",
    financialSummary: "Full Financial Summary",
    jumiaOutletsVolume: "Jumia Outlets Volume Comparison",
    genderPurchasesTitle: "Male vs Female Purchase Power",
    genderPurchasesDesc: "Detailed purchase power comparison based on successful picked-up orders.",
    totalPurchasedVal: "Total Spend",
    pickedUpOrdersCount: "Purchased Orders",
    avgPurchaseBasket: "Avg Basket Size",
    whatPurchasedTitle: "What they Purchased (Top Categories by Gender)",
    genderCompareHeader: "Male vs Female Purchases",

    // Calls Log Tab
    takeCall: "📞 Make Call",
    callMadeBy: "Call made by",
    submitResolution: "Submit Resolution",
    closeCall: "Close",
    noActiveUrgentCalls: "No urgent calls at the moment. All orders are on track.",
    callResolutionLabel: "Resolution",
    resComingPickup: "✅ Customer is coming to pick up",
    resWillCancel: "❌ Customer will cancel",
    resNoAnswer: "📵 No answer",
    resDeclined: "❓ Customer declined / wrong order",
    resNoLongerWants: "🚫 Customer no longer wants it",
    callNotes: "Notes (optional)",
    showClosed: "Show Closed",
    hideClosed: "Hide Closed",
    urgentCallsCount: "Urgent Calls",
    sourceJ: "Jumia",
    sourceBosta: "Bosta",

    // Statuses
    inventoryStatus: "Inventory",
    pickedUpStatus: "Picked Up",
    returnedStatus: "Returned",
    transactionCounters: "Transactions Counter",
    dailyTransactions: "Daily Transactions",
    weeklyTransactions: "Weekly Transactions",
    monthlyTransactions: "Monthly Transactions",
    pickedFromJumia: "Picked up from Jumia",
    pickedFromBosta: "Picked up from Bosta",
    pickedUpByCustomer: "Picked Up by customer",

    // Landing Page (Incubator)
    selectService: "Select Service Module",
    selectServiceDesc: "Choose a service to access relevant tools and management dashboards.",
    jumiaDesc: "Jumia Hub: Inventory management, SLA tracking, and customer returns.",
    bostaDesc: "Bosta Logistics: Delivery management and tracking.",
    basataDesc: "Basata POS: Financial services, top-ups, and transaction reports.",
    operationsTools: "Operations & Admin Tools",
    backToHome: "Back to Home",
    analyticsDesc: "Business intelligence and performance metrics.",
    logsDesc: "System activity and security audit logs.",
    callsDesc: "Customer support and call management.",
    
    // Imtidad C2C Logistics
    imtidadDesc: "Imtidad C2C Logistics: Ship packages between cities using our interactive pickup network.",
    imtidadShipping: "Imtidad C2C Shipping",
    c2cBooking: "Book C2C Shipment",
    c2cDashboard: "Shipments Dashboard",
    c2cAnalytics: "Network Load Analytics",
    senderName: "Sender Name",
    senderPhone: "Sender Phone",
    senderCity: "Sender City",
    dropoffOutlet: "Drop-off Station",
    recipientName: "Recipient Name",
    recipientPhone: "Recipient Phone",
    recipientCity: "Recipient City",
    pickupOutlet: "Delivery Pickup Station",
    weightKg: "Weight (kg)",
    shippingCost: "Shipping Cost",
    trackingId: "Tracking ID",
    bookingWizard: "C2C Booking Wizard",
    outletHeatmap: "Interactive Outlets Network",
    heatmapMode: "Heatmap (Volume Load)",
    normalMode: "Standard Map (Status)",
    imtidadGroup: "Imtidad Group",
    
    // Stations Management
    manageOutlets: "Manage Stations",
    addOutlet: "Add New Station",
    stationKey: "Station Key (Unique)",
    clickMapToPlace: "Click on map to automatically set coordinates",
    stationNameEn: "Station Name (English)",
    stationNameAr: "Station Name (Arabic)",
    cityEn: "City Name",
    baseLoadPercentage: "Base Load Percentage (%)",

    // Credit Status Modal
    creditStatusTitle: "SaaS Credit & Counter Status",
    creditStatusSubtitle: "Current balance and transaction counter status for this SaaS subscription",
    creditRemaining: "Credit Remaining",
    creditConsumedThisMonth: "Consumed This Month",
    totalCreditLimit: "Total Allowed Limit",
    totalLifetimeConsumed: "Total Lifetime Consumed",
    startAddingCredit: "Start Adding Credit",
    continueWithoutAdjustments: "Continue Without Adjustments",
    rechargeAmountLabel: "Recharge Amount (Operations)",
    enterAmount: "Enter amount...",
    quickSelect: "Quick Select",
    submitRechargeRequest: "Submit Recharge Request",
    adminDirectAdd: "Confirm & Add Credit Instantly",
    rechargeRequestSent: "Activation request sent. Pending admin approval.",
    creditAddedSuccessfully: "Credit added successfully! Your new limit is active.",
    operations: "operations",
    pendingApprovalsNotice: "There are pending recharge requests waiting for approval",
    viewApprovalsTab: "Go to Approvals Tab",
    breakdownTitle: "Counter Breakdown",
    channelJumia: "Jumia Orders",
    channelBosta: "Bosta Orders",
    channelBasata: "Basata POS",
    channelCustomers: "New Customers",
    systemHealthGood: "Healthy",
    systemHealthWarning: "Low Credit",
    systemHealthCritical: "Critical",
    backToOverview: "Back to Overview"
  },
  ar: {
    // Sidebar
    inventory: "مخزون الطلبات",
    customers: "العملاء",
    sla: "متابعة SLA",
    penalties: "غرامات التأخير",
    basata: "بساطة POS",
    jumia: "جوميا",
    bosta: "طلبات بوسطة",
    returned: "المرتجع لـ جوميا",
    analytics: "التحليلات",
    signOut: "تسجيل الخروج",
    jumiaStation: "محطة جوميا V3",
    fcf: "إف سي إف",
    mosaam: "مواسام",
    admin: "مدير",
    staff: "موظف",
    callsLog: "سجل المكالمات",
    rechargeApprovals: "طلبات الشحن والتفعيل",
    rechargeApprovalsDesc: "إدارة واعتماد طلبات شحن الرصيد والحدود للمستخدمين",
    logs: "سجلات النظام",
    user: "المستخدم",
    action: "الإجراء",
    details: "التفاصيل",
    allActions: "جميع الإجراءات",
     allUsers: "جميع المستخدمين",
    manageUsers: "إدارة المستخدمين",
    userBranchAssignment: "تعيين فروع المستخدمين",
    assignBranch: "تعيين فرع",
    assignedBranch: "الفرع المعين",
    selectBranch: "اختر الفرع",
    successUpdateUser: "تم تحديث فرع المستخدم بنجاح",

    // Common
    confirm: "تأكيد",
    cancel: "إلغاء",
    search: "بحث...",
    loading: "جاري تحميل إف سي إف مواسام...",
    successfullyLogged: "تم التسجيل بنجاح!",
    errorLogging: "خطأ في تسجيل المعاملة",
    noData: "لا توجد بيانات.",
    date: "التاريخ",
    amount: "المبلغ",
    status: "الحالة",
    actions: "الإجراءات",
    exportReport: "تصدير تقرير",
    importData: "استيراد بيانات",
    total: "الإجمالي",
    save: "حفظ",
    delete: "حذف",
    edit: "تعديل",
    view: "عرض",
    all: "الكل",
    from: "من",
    to: "إلى",
    custom: "فترة مخصصة",

    // Orders Tab
    receiveNewOrder: "استلام طلب جديد",
    bulkImport: "استيراد جماعي (Excel/CSV)",
    orderId: "رقم الطلب",
    customer: "العميل",
    phone: "الهاتف",
    description: "الوصف",
    value: "القيمة",
    category: "الفئة",
    receivedAt: "تاريخ الاستلام",
    markPickedUp: "تم الاستلام",
    markReturned: "تم الارجاع",
    daysInInv: "أيام في المخزن",
    penalty: "الغرامة",
    inventorySummary: "ملخص المخزون",
    parkedOrders: "الطلبات المتوقفة",
    totalValue: "إجمالي القيمة",
    totalPenalties: "إجمالي الغرامات",
    packageSize: "حجم الطرد",
    small: "صغير",
    medium: "متوسط",
    big: "كبير",

    // Customers Tab
    customerDirectory: "دليل العملاء",
    addCustomer: "إضافة عميل جديد",
    name: "الاسم",
    email: "البريد الإلكتروني",
    address: "العنوان",
    tier: "الفئة",
    deliveries: "تسليمات جوميا",
    bostaDeliveries: "تسليمات بوسطة",
    newCustomer: "عميل جديد",
    loyaltyTier: "فئة الولاء",
    customerInfo: "معلومات العميل",

    // Basata Tab
    logPosTransaction: "تسجيل معاملة POS",
    popularQuickActions: "إجراءات سريعة شائعة (شحن رصيد)",
    allServiceCategories: "جميع فئات الخدمات",
    transactionReport: "تقرير المعاملات",
    serviceProvider: "مزود الخدمة",
    transactionId: "رقم المعاملة",
    paymentMethod: "طريقة الدفع",
    percentage: "النسبة %",
    totalVolume: "إجمالي الحجم",
    method: "الطريقة",
    service: "الخدمة",
    confirmLogging: "تأكيد التسجيل",
    cash: "نقداً",
    card: "بطاقة",
    wallet: "محفظة رقمية",
    other: "أخرى",
    bankingAndCash: "خدمات بنكية وكاش",
    cashDeposit: "إيداع نقدي",
    cashWithdrawal: "سحب نقدي",
    visa: "فيزا",
    creditCard: "بطاقة ائتمان",
    jumiaPay: "جوميا باي",
    eltalg: "وبور التلج (eltalg)",
    tegara: "تجارة (tegara)",
    mostashfa: "المستشفى (mostashfa)",

    // Bosta Tab
    bostaInventory: "مخزون طلبات بوسطة",
    receiveBostaOrder: "استلام طلب بوسطة",

    // SLA Tab
    slaMonitoring: "مراقبة SLA وتتبع التأخير",
    slaAlerts: "تنبيهات SLA",
    critical5Days: "حرج (5+ أيام)",
    warning3Days: "تحذير (3-4 أيام)",
    onTrack: "في المسار",

    // Parked Penalties Tab
    parkedPenalties: "الطلبات المتوقفة والغرامات اليومية",
    parkedDescription: "نظرة عامة على الطلبات المتوقفة حالياً في المحطة وغرامات تأخير الاستلام المتراكمة.",

    // Returned Tab
    returnedOrders: "المرتجع لـ جوميا",
    returnDate: "تاريخ الارجاع",

    // Customer Returns Section (in Orders Tab)
    customerReturns: "مرتجعات العملاء",
    customerReturnsDesc: "الطرود التي أرجعها العملاء للمحطة بانتظار إرجاعها لـ جوميا.",
    receiveCustomerReturn: "استلام مرتجع عميل",
    returnReason: "السبب",
    atStation: "في المحطة",
    returnedToJumia: "تم الارجاع لـ جوميا",
    markReturnedToJumia: "تم الارجاع لـ جوميا",
    pendingReturns: "مرتجعات معلقة",

    // Analytics Tab
    performance: "الأداء",
    daily: "يومي",
    weekly: "أسبوعي",
    monthly: "شهري",
    totalStationRevenue: "إجمالي إيرادات المحطة",
    revenueStreams: "تحليل مصادر الإيرادات",
    ordersComparison: "مقارنة طلبات جوميا وبوسطة",
    ordersStatusDist: "توزيع حالة الطلبات بالكامل",
    slaHealth: "صحة SLA لـ جوميا",
    basataRevenueCat: "إيرادات بساطة حسب الفئة",
    topBasataServices: "أعلى خدمات بساطة حسب الحجم",
    financialSummary: "الملخص المالي الكامل",
    jumiaOutletsVolume: "مقارنة حجم طلبات جوميا حسب المنفذ",
    genderPurchasesTitle: "مقارنة القوة الشرائية (رجال vs نساء)",
    genderPurchasesDesc: "مقارنة تفصيلية للقوة الشرائية بناءً على الطلبات المستلمة بنجاح.",
    totalPurchasedVal: "إجمالي الإنفاق",
    pickedUpOrdersCount: "الطلبات المستلمة",
    avgPurchaseBasket: "متوسط حجم السلة",
    whatPurchasedTitle: "ماذا اشتروا (أعلى الفئات حسب النوع)",
    genderCompareHeader: "مقارنات الشراء للرجال والنساء",

    // Calls Log Tab
    takeCall: "📞 إجراء المكالمة",
    callMadeBy: "تم الاتصال بواسطة",
    submitResolution: "تأكيد النتيجة",
    closeCall: "إغلاق",
    noActiveUrgentCalls: "لا توجد مكالمات عاجلة حالياً. جميع الطلبات في المسار.",
    callResolutionLabel: "النتيجة",
    resComingPickup: "✅ العميل سيأتي للاستلام",
    resWillCancel: "❌ العميل سيلغي الطلب",
    resNoAnswer: "📵 لا يوجد رد",
    resDeclined: "❓ العميل رفض / طلب خاطئ",
    resNoLongerWants: "🚫 العميل لا يريد الطلب بعد الآن",
    callNotes: "ملاحظات (اختياري)",
    showClosed: "عرض المغلقة",
    hideClosed: "إخفاء المغلقة",
    urgentCallsCount: "مكالمات عاجلة",
    sourceJ: "جوميا",
    sourceBosta: "بوسطة",

    // Statuses
    inventoryStatus: "في المخزن",
    pickedUpStatus: "تم الاستلام",
    returnedStatus: "تم الارجاع",
    transactionCounters: "عداد المعاملات",
    dailyTransactions: "المعاملات اليومية",
    weeklyTransactions: "المعاملات الأسبوعية",
    monthlyTransactions: "المعاملات الشهرية",
    pickedFromJumia: "استلام من جوميا",
    pickedFromBosta: "استلام من بوسطة",
    pickedUpByCustomer: "استلام من قبل العميل",

    // Landing Page (Incubator)
    selectService: "اختر وحدة الخدمة",
    selectServiceDesc: "اختر خدمة للوصول إلى الأدوات ذات الصلة ولوحات إدارة العمليات.",
    jumiaDesc: "مركز جوميا: إدارة المخزون، تتبع SLA، ومرتجعات العملاء.",
    bostaDesc: "لوجستيات بوسطة: إدارة وتسليم الطرود وتتبعها.",
    basataDesc: "بساطة POS: الخدمات المالية، الشحن، وتقارير المعاملات.",
    operationsTools: "أدوات العمليات والإدارة",
    backToHome: "العودة للرئيسية",
    analyticsDesc: "تحليل البيانات ومقاييس الأداء.",
    logsDesc: "سجلات نشاط النظام والتدقيق الأمني.",
    callsDesc: "إدارة اتصالات وخدمة العملاء.",
    
    // Imtidad C2C Logistics
    imtidadDesc: "خدمات امتداد اللوجستية: شحن الطرود من شخص لشخص (C2C) عبر شبكة محطاتنا التفاعلية.",
    imtidadShipping: "شحن امتداد C2C",
    c2cBooking: "حجز شحنة C2C",
    c2cDashboard: "لوحة تتبع الشحنات",
    c2cAnalytics: "تحليلات حمولة الشبكة",
    senderName: "اسم المرسل",
    senderPhone: "رقم هاتف المرسل",
    senderCity: "مدينة المرسل",
    dropoffOutlet: "محطة الإيداع",
    recipientName: "اسم المستلم",
    recipientPhone: "رقم هاتف المستلم",
    recipientCity: "مدينة المستلم",
    pickupOutlet: "محطة الاستلام والتسليم",
    weightKg: "الوزن (كجم)",
    shippingCost: "تكلفة الشحن",
    trackingId: "رقم التتبع",
    bookingWizard: "معالج الحجز C2C",
    outletHeatmap: "شبكة الفروع التفاعلية",
    heatmapMode: "الخريطة الحرارية (حمولة الطرود)",
    normalMode: "الخريطة العادية (حالة الفروع)",
    imtidadGroup: "مجموعة امتداد",
    
    // Stations Management
    manageOutlets: "إدارة المحطات",
    addOutlet: "إضافة محطة جديدة",
    stationKey: "رمز المحطة (فريد)",
    clickMapToPlace: "انقر على الخريطة لتحديد الإحداثيات تلقائياً",
    stationNameEn: "اسم المحطة (بالإنجليزية)",
    stationNameAr: "اسم المحطة (بالعربية)",
    cityEn: "اسم المدينة",
    baseLoadPercentage: "نسبة الحمولة الأساسية (%)",

    // Credit Status Modal
    creditStatusTitle: "حالة رصيد وعداد النظام (SaaS)",
    creditStatusSubtitle: "حالة الرصيد المتبقي وعداد العمليات الفعلي لاشتراك النظام",
    creditRemaining: "الرصيد المتبقي",
    creditConsumedThisMonth: "المستهلك هذا الشهر",
    totalCreditLimit: "إجمالي الحد المسموح به",
    totalLifetimeConsumed: "إجمالي المستهلك حتى الآن",
    startAddingCredit: "بدء عملية شحن الرصيد",
    continueWithoutAdjustments: "المتابعة بدون إجراء تعديل",
    rechargeAmountLabel: "مبلغ الشحن (عدد العمليات)",
    enterAmount: "أدخل كمية العمليات...",
    quickSelect: "تحديد سريع",
    submitRechargeRequest: "إرسال طلب الشحن",
    adminDirectAdd: "تأكيد وإضافة الرصيد فوراً",
    rechargeRequestSent: "تم إرسال طلب التفعيل بنجاح. في انتظار موافقة الإدارة.",
    creditAddedSuccessfully: "تمت إضافة الرصيد بنجاح! تم تفعيل الحد الجديد فوراً.",
    operations: "عملية",
    pendingApprovalsNotice: "توجد طلبات شحن معلقة في انتظار اعتمادك",
    viewApprovalsTab: "الانتقال لتبويب الاعتمادات",
    breakdownTitle: "تفاصيل استهلاك العداد",
    channelJumia: "طلبات جوميا",
    channelBosta: "طلبات بوسطة",
    channelBasata: "معاملات بساطة",
    channelCustomers: "العملاء الجدد",
    systemHealthGood: "رصيد وفير",
    systemHealthWarning: "رصيد منخفض",
    systemHealthCritical: "رصيد حرج",
    backToOverview: "الرجوع للملخص"
  }
};
